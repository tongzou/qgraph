...
class NodeBehavior extends Behavior {
  constructor (renderer) {
    ...
    dispatcher.register(['mousedown.*', 'zoom*'], function (type, ns, data, pos, event) {
      ...
      if (!dom.eventFromInput(event)) { Label.stopEditor() }
    }, this)
  }
  ...
  handleDrag (type, ns, vertex, pos, event) {
    ...
    if (type == 'dragstart') {
      ...
      var offsets = this.offsets = []; var edges = []
      ...
      if (this.graph.getProperty('guidesEnabled') || this.graph.getProperty('gridEnabled')) { this.guide = new Guide(this.graph, cells, pos) }
      this.dragNodes = this.renderer.getShapes(cells)
      this.dragLinks = this.renderer.getShapes(edges)
      return
    }
    if (this.guide) { pos = this.guide.move(pos) }
    ...
    if (!t2[0] && !t2[1]) {
      ...
      if (this.dragLinks && this.dragLinks.length > 0) { this.renderer.refreshLinks(this.dragLinks) }
    } else {
      this.stop = false
      if (!this.fn) {
        this.fn = function loopingFunction () {
          if (!t2[0] && !t2[1] || self.stop) {
            console.log('No Loop anymore')
            delete self.lastPos
            delete self.stop
            delete self.fn
            return
          ...
          _.each(cells, function (cell, i) {
            ...
            // console.log("To: " + (self.lastPos[0]) + " " + (self.lastPos[1]))
            cell.move(p.x - t2[0], p.y - t2[1])// self.lastPos[0], self.lastPos[1]);
          }, self)

          self.renderer.refreshNodes(self.dragNodes)

          if (self.dragLinks && self.dragLinks.length > 0) { self.renderer.refreshLinks(self.dragLinks) }
  ...
  handleMouseEnter (type, ns, vertex, pos, event) {
    ...
    if (this.connectionMarker && this.connectionMarker.vertex != vertex) { this.hideConnectionMarker() }

    if (!this.connectionMarker) { this.showConnectionMarker(vertex) }
  ...
  handleConnection (type, ns, data, pos, event) {
    ...
    if (!isValid && (isValid || newVertex == null)) { this.renderer.refreshLinks(this._newLink) }
  }
  ...
  showConnectionMarker (vertex) {
    ...
    var mousemove = _.bind(function () {
      ...
      if (!data.bounds.contains(new Point(pos[0], pos[1]))) { this.hideConnectionMarker() }
    }, this)
  ...
  hideConnectionMarker () {
    ...
    if (this._w) { this._w.on('mousemove', null) }
  }
